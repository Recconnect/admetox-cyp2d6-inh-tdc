...
class GraphBank:
    """Pre-concatenated GPU graph bank for fast batching.

    All node/edge feature tensors are concatenated into single flat GPU
    arrays once; a batch is then built with cheap slice-cats instead of
    per-graph CPU->GPU copies (the original bottleneck: ~56ms/batch @ bs=64
    -> ~2ms). Batching semantics are numerically identical to batch_tensors.
    """

    def __init__(self, graphs, device):
        import numpy as np
        g = graphs
        counts = np.array([x['n_nodes'] for x in g], dtype=np.int64)
        ecounts = np.array([x['src'].numel() for x in g], dtype=np.int64)
        self.ns = np.concatenate([[0], np.cumsum(counts)]).astype(np.int64)
        self.es = np.concatenate([[0], np.cumsum(ecounts)]).astype(np.int64)
        self.atom_id = torch.cat([x['atom_id'] for x in g]).to(device)
        self.chir = torch.cat([x['chir'] for x in g]).to(device)
        self.bond_type = torch.cat([x['bond_type'] for x in g]).to(device)
        self.bond_dir = torch.cat([x['bond_dir'] for x in g]).to(device)
        src = torch.cat([x['src'] for x in g])
        dst = torch.cat([x['dst'] for x in g])
        off = torch.repeat_interleave(torch.as_tensor(self.ns[:-1]),
                                      torch.as_tensor(ecounts))
        self.src = (src + off).to(device)
        self.dst = (dst + off).to(device)
        self.device = device

    def batch(self, ids):
        ns, es = self.ns, self.es
        ids = list(ids)
        n_sizes = torch.as_tensor([ns[i + 1] - ns[i] for i in ids], dtype=torch.int64)
        e_sizes = torch.as_tensor([es[i + 1] - es[i] for i in ids], dtype=torch.int64)
        gi = torch.repeat_interleave(torch.arange(len(ids), device=self.device),
                                     n_sizes.to(self.device))
        run_off = torch.cumsum(torch.cat([torch.zeros(1, dtype=torch.int64),
                                          n_sizes[:-1]]), 0)
        g_off = torch.repeat_interleave(torch.as_tensor(ns[ids], device=self.device),
                                        e_sizes.to(self.device))
        l_off = torch.repeat_interleave(run_off, e_sizes).to(self.device)
        parts_n = [slice(ns[i], ns[i + 1]) for i in ids]
        parts_e = [slice(es[i], es[i + 1]) for i in ids]
        atom = torch.cat([self.atom_id[s] for s in parts_n])
        chir = torch.cat([self.chir[s] for s in parts_n])
        bt = torch.cat([self.bond_type[s] for s in parts_e])
        bd = torch.cat([self.bond_dir[s] for s in parts_e])
        s2 = torch.cat([self.src[s] for s in parts_e]) - g_off + l_off
        d2 = torch.cat([self.dst[s] for s in parts_e]) - g_off + l_off
        return {"atom_id": atom, "chir": chir, "bond_type": bt, "bond_dir": bd,
                "src": s2, "dst": d2, "graph_idx": gi, "n_nodes": int(atom.numel())}
